  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[2rem] border border-gray-200 bg-white p-6">
        ...
        {!hasOrders && !hasProducts ? <p className="mt-2 text-sm text-gray-500">{empty}</p> : null}
      </div>

      {hasOrders ? (
        <div className="grid gap-4 lg:grid-cols-2">
          {orders?.map((order) => (
            <div key={order.id} className="rounded-[1.75rem] border border-gray-200 bg-white p-4">
              <div className="flex gap-3">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-semibold text-gray-900">{order.product.title}</div>
                  <div className="mt-1 text-sm text-gray-500">{order.orderStatus} • {order.paymentStatus}</div>
                </div>
              </div>
            </div>
          ))}
      ...
      {hasProducts ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {products?.map((product) => (
            <div key={product.id} className="rounded-[1.75rem] border border-gray-200 bg-white p-4">
              <div className="text-sm font-semibold text-gray-900">{product.title}</div>
              <div className="mt-1 text-sm text-gray-500">{product.status}</div>
            </div>
          ))}
        </div>
      ) : null}
